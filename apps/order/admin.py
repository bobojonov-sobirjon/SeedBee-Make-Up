...
class CardDetailsAdmin(admin.ModelAdmin):
    list_display = ('id', 'user_email', 'card_holder', 'card_number_masked', 'expiration_date', 'created_at', 'is_expired')
...
class OrderAdmin(admin.ModelAdmin):
    list_display = ('order_id', 'user_email', 'total_price', 'products_count', 'created_at', 'status_display')
    list_filter = ('created_at', 'user', 'total_price')
    ...
    fieldsets = (
        (_('Основная информация'), {
            'fields': ('order_id', 'user', 'total_price')
        }),
        (_('Продукты'), {
            'fields': ('products', 'products_display'),
            'classes': ('collapse',)
        }),
        (_('Системная информация'), {
            'fields': ('created_at',),
    ...
    def products_display(self, obj):
        ...
        for product in obj.products:
            ...
            html += f'<li><strong>{product_name}</strong> (ID: {product_id}) - '
